...
// --- DSP ENGINE (SMI11 mit Persistent Socket) ---
    ...
    while (file.read(reinterpret_cast<char*>(&chunk), sizeof(ChunkHeader))) {
        ...
        if (tag == "fmt ") {
            ...
            if (chunk.size >= sizeof(AuxiContent)) {
                ...
                //std::string rawName(aux->filename);
                ...
                if (last != std::string::npos) {
                    nextFileFound = rawName.substr(0, last + 1);
                         ...
                         // Falls der String nur aus Müll bestand:
                      nextFileFound = "";
                }
            }
            ...
            while (file.read(reinterpret_cast<char*>(readBuf.data()), blockSize * 4)) {
                ...
                int ch = getch();
                if (ch == 'q' || ch == 'Q') { nextFileFound = ""; break; }
                ...
                if (++ui_divider >= 200) {
                    ...
                    mvprintw(1, 2, ">>> SENDET (GAPLESS): %s", filename.c_str());
                    attroff(COLOR_PAIR(1) | A_BOLD);
                    mvprintw(3, 2, "Rate: %.2f MHz | Shift: %.1f kHz", (float)sampleRate/1e6, centerFreq/1e3);

                    int barLen = (int)((peak_hold / 1.0f) * 40.0f);
                    ...
                    printw("] %.1f%%", peak_hold * 100.0f);

                    float progress = (float)bytesRead / (float)dataSize * 100.0f;
                    mvprintw(6, 2, "Forts: [");
                    int progLen = (int)(progress / 100.0f * 40.0f);
                    for(int b=0; b<progLen; b++) addch('=');
                    for(int b=progLen; b<40; b++) addch(' ');
                    printw("] %.1f%%", progress);

                    if (!nextFileFound.empty()) mvprintw(8, 2, "naechste Datei: %s", nextFileFound.c_str());
...
int main(int argc, char* argv[]) {
    ...
    while(true) {
        auto entries = get_directory_content(currentPath);
        if (highlight >= entries.size()) highlight = 0;
        int maxRows = LINES - 6;

        clear();
        attron(COLOR_PAIR(3) | A_REVERSE);
        mvprintw(0, 0, " IP: %s | BITS: %-2d | [i] IP | [b] Bits | [q] Exit ", targetIP.c_str(), targetBits);
        attroff(COLOR_PAIR(3) | A_REVERSE);
        mvprintw(1, 0, " Pfad: %s", currentPath.string().c_str());
        ...
        else if(c == 'i') { targetIP = get_string_input(LINES-2, 0, "Ziel-IP", targetIP); }
        else if(c == 'b') {
            std::string bStr = get_string_input(LINES-2, 0, "Bitbreite", std::to_string(targetBits));
            try {
                 targetBits = std::stoi(bStr);
                 if (targetBits == 8) {
                   set_dac_width(targetIP, 8);
                 } else {
                   set_dac_width(targetIP,16);
                 }
            } catch(...) {}
        }
        else if(c == 10) { // ENTER
            if(entries[highlight].name == "..") { currentPath = currentPath.parent_path(); highlight = 0; scrollOffset = 0; }
            else if(entries[highlight].is_dir) { currentPath /= entries[highlight].name; highlight = 0; scrollOffset = 0; }
            else {
                ...
                if (connect(sock, (struct sockaddr *)&srv, sizeof(srv)) >= 0) {
                    ...
                    while (!nxt.empty()) {
                        ...
                        if (!nxt.empty() && !fs::exists(currentPath / nxt)) break;
                    }
                    close(sock);
                } else {
                    mvprintw(LINES-2, 2, "Verbindung zu %s gescheitert!", targetIP.c_str());
                    refresh(); usleep(1500000);
                    close(sock);
                }
