...
class App:
        ...
        while self.running:
            try:
                ...
                while self.running:
                    ...
                    while len(raw_buffer) >= (FFT_SIZE * 2):
                        data_block = raw_buffer[:(FFT_SIZE * 2)]
                        del raw_buffer[:(FFT_SIZE * 2)]
                        self.data_queue.append(np.frombuffer(data_block, dtype=np.int16).astype(np.float32))
                conn.close()
    ...
    def update_plot(self):
        if self.data_queue:
            samples = self.data_queue.popleft() / 32768.0
